...
class Solution {
    public int strToInt(String str) {
        /**
         * 这个题目最大的问题在于怎么判断超过32位越界的问题，题目说可以直接返回Integer的极限值
         * 那就剩一个判断了，极大值的情况是Integer.MAX_VALUE且最有一位是7，那么可以直接判断不算最后一位的情况
         *
         */
        //i,用来处理前面空格和负号的遍历符;sign符号标识位;res结果;边界值
        int i = 0, sign = 1, len = str.length(), res = 0, bonary = Integer.MAX_VALUE / 10;
        if (len == 0)
            return 0;
        while (str.charAt(i) == ' ')
            if (++i == len)
        ...
        for (int j = i; j < len; j++) {
            if (str.charAt(j) < '0' || str.charAt(j) > '9')
                break;
            if (res > bonary || res == bonary && str.charAt(j) > '7')
